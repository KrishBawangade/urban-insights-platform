...
load_dotenv()
TRAFFIC_AI_CACHE_TTL_SECONDS = 300
...
def generate_ai_insight(overview_data: Dict[str, Any], hourly_trend: List[Dict[str, Any]], prediction_data: List[Dict[str, Any]]):
    ...
    """
    prediction_sample = tuple(
        (item.get("junction"), item.get("predicted_vehicles"))
        for item in prediction_data[:3]
    )
    cache_key = (
        "traffic:ai-insight:"
        f"{overview_data.get('congestion_index')}:"
        f"{overview_data.get('average_speed')}:"
        f"{overview_data.get('active_hotspots')}:"
        f"{overview_data.get('vehicles_per_minute')}:"
        f"{prediction_sample}"
    )
    ...
    return cache.get_or_set(cache_key, TRAFFIC_AI_CACHE_TTL_SECONDS, build_insight)
